...
const getSortedTracks = (onchainTracks: Track[], firebaseTracks: Track[]) => {
  ...
  const orderMap = new Map(
    firebaseTracks.map((item, index) => [item.token.contractAddress + item.token.id, index]),
  )

  const sortedTracks = onchainTracks
    .filter((track: Track) => orderMap.has(track.token.contractAddress + track.token.id))
    .sort((a: any, b: any) => {
  ...
  const additionalTracks = onchainTracks.filter(
    (track: Track) => !orderMap.has(track.token.contractAddress + track.token.id),
  )
