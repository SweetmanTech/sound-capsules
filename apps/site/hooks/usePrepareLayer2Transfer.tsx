...
const usePrepareLayer2Transfer = () => {
  ...
  const registerAndInit = async (toastId: any, chainId: any) => {
    try {
      ...
      const isInitialized = await getImplementationAddress(capsuleAddress, chainId)
      if (!isDeployed || !isInitialized) {
        ...
        const hash = await aggregate3Value([registryCall, initCall], getViemNetwork(chainId))
