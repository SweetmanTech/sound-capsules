...
export default function TrackGrid() {
  ...
  if (nfts.length <= 0) {
    return (
      <div className="text-center max-w-xl mx-auto py-24 space-y-1">
        ...
        <p>
          There are no audio NFTs in your wallet we can detect, you can only send NFTs from the
          owner wallet of your Capsule.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
      {nfts?.map((track) => (
        <TrackGridItem track={track} key={`${track.token.contractAddress}-${track.token.id}`} />
      ))}
    </div>
  )
}
