        ...
        <section className="border-t pt-4 space-y-16">
          <div className="flex justify-between items-start md:items-center">
            <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4">
              <h2 className="font-size-text text-black">{capsule.meta.name ?? "Capsule wallet"}</h2>
            ...
            {editing ? (
              <div className="flex gap-2">
                ...
                <button onClick={handleClick} className="py-2 px-4 bg-black text-white rounded-2xl">
                  Save order
                </button>
              </div>
            ) : (
          ...
          {orderedTracks.length > 0 ? (
            <Fragment>{editing ? <DragNDropReorder /> : <CapsuleTrackList />}</Fragment>
          ) : (
            <div className="text-center max-w-xl mx-auto py-24 space-y-2">
              <div className="font-size-text text-black">Empty</div>
              <p>
                There are no audio NFTs in your Sound Capsule smart wallet, transfer audio NFTs from
                your wallet to add them to your playlist.
              </p>
            </div>
          )}
        </section>
